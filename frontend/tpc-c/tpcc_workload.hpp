...
template<class Record, class Field>
static auto lookupField(Adapter<Record> * adapter, const typename Record::Key& key, Field Record::*f) {
   Field local_f;
   adapter->lookup1(key, [&](const Record & payload){
      local_f = (payload).*f;
   });
   return local_f;
}
...
void newOrder(Integer w_id,
              ...
              Timestamp timestamp)
{
   Numeric w_tax = lookupField<warehouse_t>(warehouse, {w_id}, &warehouse_t::w_tax);
   Numeric c_discount = lookupField<customer_t>(customer, {w_id, d_id, c_id}, &customer_t::c_discount);
   Numeric d_tax;
   Integer o_id;
   ...
   for (unsigned i = 0; i < lineNumbers.size(); i++) {
      ...
      Numeric i_price = lookupField<item_t>(item, {itemid}, &item_t::i_price);  // TODO: rollback on miss
      Varchar<24> s_dist;
...
{
   Integer o_id = lookupField<district_t>(district, {w_id, d_id}, &district_t::d_next_o_id);
 ...
 s_i_id=ol_i_id AND s_quantity < :threshold;
   ...
   for (Integer i_id : items) {
      auto res_s_quantity = lookupField<stock_t>(stock, {w_id, i_id}, &stock_t::s_quantity);
