...
#ifndef STX_STX_COMPRESSED_BTREE_H_HEADER
#define STX_STX_COMPRESSED_BTREE_H_HEADER
...
// Leaf-compressed BTree implementation by Xinjing Zhou on 06/20/2022
...
public:
    ...
    typedef compressed_btree<key_type, data_type, value_type, key_compare,
...
private:
    ...
    struct inner_node : public node
    {
        ...
        key_type & get_key(unsigned short slot) {
            return slotkey[slot];
        }

        node *& get_value(unsigned short slot) {
            return childid[slot];
        }

        const key_type & get_key(unsigned short slot) const {
            return slotkey[slot];
        }

        const node *& get_value(unsigned short slot) const{
            return childid[slot];
        }

        key_type * get_slotkey_array() {
            return slotkey;
        }

        node ** get_slotdata_array() {
            return childid;
        }
    };
    ...
    char compression_scratch_pad[sizeof(leaf_node_data) * 2];
    ...
    public:
        ...
        self_type* btree = nullptr;
        mutable uint64_t data_ptr; // Compressed or decompressed.
        mutable uint32_t compressed_size;
        
        leaf_node_data * get_node_data() const {
            assert(data_ptr != 0);
            if (data_ptr & 1) {
                // TODO decompress
                ...
                btree->m_stats.leaf_node_data_bytes += sizeof(leaf_node_data);
                btree->m_stats.leaf_node_data_bytes -= compressed_size;
                compressed_size = 0;
                assert((data_ptr & 1) == 0);
                btree->m_stats.uncompressed_leaves++;
                btree->m_stats.compressed_leaves--;
                return decompressed_node;
            } else {
                return reinterpret_cast<leaf_node_data*>(data_ptr);
            }
        }

        char* strip_off_data_ptr() const{
            return (char*)(data_ptr & ~1);
        }

        ~leaf_node() {
            assert(data_ptr > 0);
            if (data_ptr & 1) {
                free(strip_off_data_ptr());
                btree->m_stats.leaf_node_data_bytes -= compressed_size;
                btree->m_stats.compressed_leaves--;
            } else {
                delete reinterpret_cast<leaf_node_data*>(data_ptr);
                btree->m_stats.leaf_node_data_bytes -= sizeof(leaf_node_data);
                btree->m_stats.uncompressed_leaves--;
            }
        ...
        void ensure_compressed() {
            assert(data_ptr != 0);
            if (!(data_ptr & 1)) { // uncompressed form
                // TODO compress
                auto comp_size = LZ4_compress_default((const char *)data_ptr, btree->compression_scratch_pad, sizeof(leaf_node_data), sizeof(btree->compression_scratch_pad));
                char* compressed_buf = (char*)malloc(comp_size);
                memcpy(compressed_buf, btree->compression_scratch_pad, comp_size);
                compressed_size = comp_size;
                delete reinterpret_cast<leaf_node_data*>(data_ptr);
                data_ptr = ((uint64_t)compressed_buf) | 1;
                btree->m_stats.leaf_node_data_bytes -= sizeof(leaf_node_data);
                btree->m_stats.leaf_node_data_bytes += compressed_size;
                assert(data_ptr & 1);
                btree->m_stats.compressed_leaves++;
        ...
        key_type& get_key(unsigned short slot) {
            ...
            return get_node_data()->slotkey[slot];
        }

        data_type& get_value(unsigned short slot) {
            BTREE_ASSERT(used_as_set == false);
            BTREE_ASSERT(slot < node::slotuse);
            return get_node_data()->slotdata[slot];
        }
        ...
        const data_type& get_value(unsigned short slot) const {
            BTREE_ASSERT(used_as_set == false);
            BTREE_ASSERT(slot < node::slotuse);
            return get_node_data()->slotdata[slot];
        }
        ...
        key_type* get_slotkey_array() {
            return get_node_data()->slotkey;
        }

        data_type* get_slotdata_array() {
            return get_node_data()->slotdata;
        }

        /// Set variables to initial values
        inline leaf_node(self_type* tree)
            : node(0), prevleaf(NULL), nextleaf(NULL), btree(tree), data_ptr(0), compressed_size(0)
        { 
            leaf_node_data* node_data_ptr = new leaf_node_data; 
            memset(node_data_ptr, 0, sizeof(leaf_node_data)); 
            data_ptr = (uint64_t)(node_data_ptr);
            btree->m_stats.leaf_node_data_bytes += sizeof(leaf_node_data);
            btree->m_stats.uncompressed_leaves++;
        }
        ...
        inline leaf_node(const node& top)
            : node(top.level, top.slotuse), prevleaf(NULL), nextleaf(NULL), data_ptr(0), compressed_size(0)
        { 
            assert(false);
            auto node_data_ptr = new leaf_node_data; 
            memset(node_data_ptr, 0, sizeof(leaf_node_data)); 
            data_ptr = (uint64_t)(node_data_ptr);
            btree->m_stats.leaf_node_data_bytes += sizeof(leaf_node_data);
            btree->m_stats.uncompressed_leaves++;
        ...
        inline void set_slot(unsigned short slot, const pair_type& value)
        {
            ...
            get_node_data()->slotkey[slot] = value.first;
            get_node_data()->slot_data[slot] = value.second;
        }
        ...
        inline void set_slot(unsigned short slot, const key_type& key)
        {
            ...
            BTREE_ASSERT(slot < node::slotuse);
            get_node_data()->slotkey[slot] = key;
        }
    };
...
public:
    ...
    public:
        ...
        typedef typename compressed_btree::key_type key_type;

        /// The data type of the btree. Returned by data().
        typedef typename compressed_btree::data_type data_type;

        /// The value type of the btree. Returned by operator*().
        typedef typename compressed_btree::value_type value_type;

        /// The pair type of the btree.
        typedef typename compressed_btree::pair_type pair_type;
    ...
    private:
        ...
        typename compressed_btree::leaf_node * currnode;
        ...
        friend class compressed_btree<key_type, data_type, value_type, key_compare,
    ...
    public:
        ...
        inline iterator(typename compressed_btree::leaf_node* l, unsigned short s)
        ...
        {
            return currnode->get_key(currslot);
        }

        void ensure_node_compressed() {
            if (currnode != nullptr) {
                currnode->ensure_compressed();
            }
        }
        ...
        {
            return currnode->get_value(used_as_set ? 0 : currslot);
        }
        ...
        inline iterator& operator ++ ()
        {
            if (currslot + 1 < currnode->slotuse) {
                ...
                // compress the current node before switching to the next one
                currnode->ensure_compressed();
    ...
    private:
        ...
        /// The currently referenced leaf node of the tree
        const typename compressed_btree::leaf_node * currnode;
    ...
    public:
        ...
        inline const_iterator(const typename compressed_btree::leaf_node* l, unsigned short s)
            : currnode(l), currslot(s)
        { }
    ...
    public:
        ...
        inline reverse_iterator(typename compressed_btree::leaf_node* l, unsigned short s)
            : currnode(l), currslot(s)
        { }
        ...
        {
            BTREE_ASSERT(currslot > 0);
            return currnode->get_key(currslot - 1);
        }
        ...
        {
            BTREE_ASSERT(currslot > 0);
            return currnode->get_value(used_as_set ? 0 : currslot - 1);
        }
    ...
    public:
        ...
        inline const_reverse_iterator(const typename compressed_btree::leaf_node* l, unsigned short s)
            : currnode(l), currslot(s)
        { }
...
public:
    ...
    struct tree_stats
    {
        ...
        /// Number of bytes in all leaf_node_data structures (compressed or uncompressed)
        size_type                   leaf_node_data_bytes;

        size_type                   compressed_leaves;
        size_type                   uncompressed_leaves;
        ...
        inline tree_stats()
            : itemcount(0),
              leaves(0), innernodes(0), leaf_node_data_bytes(0), compressed_leaves(0), uncompressed_leaves(0)
        { }
        ...
        inline size_t inner_node_size() const {
            return sizeof(inner_node);
        }

        inline size_t leaf_node_size() const {
        ...
        inline size_t  tree_total_bytes() const {
            return leaf_node_data_bytes + leaf_node_size() * leaves + innernodes * inner_node_size();
        }

        inline size_t  uncompressed_tree_total_bytes() const {
            return (sizeof(leaf_node) + sizeof(leaf_node_data)) * leaves + innernodes * inner_node_size();
        }
    };
...
public:
    ...
    /// comparison function
    explicit inline compressed_btree(const allocator_type& alloc = allocator_type())
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL), m_allocator(alloc)
    { }

    /// Constructor initializing an empty B+ tree with a special key
    /// comparison object
    explicit inline compressed_btree(const key_compare& kcf,
    ...
    inline compressed_btree(InputIterator first, InputIterator last,
                 const allocator_type& alloc = allocator_type())
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL), m_allocator(alloc)
    {
        insert(first, last);
    ...
    template <class InputIterator>
    inline compressed_btree(InputIterator first, InputIterator last, const key_compare& kcf,
                 const allocator_type& alloc = allocator_type())
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL),
          m_key_less(kcf), m_allocator(alloc)
    {
...
private:
    ...
    {
        // if (0 && sizeof(n->slot_key) > traits::binsearch_threshold)
        // {
            ...
            //     if (key_lessequal(key, n->get_key[mid])) {
            //         hi = mid;     // key <= mid
            //     }
            //     else {
            //         lo = mid + 1; // key > mid
            ...
            // {
            //     int i = 0;
            //     while (i < n->slotuse && key_less(n->get_key[i], key)) ++i;
        ...
        //{
            int lo = 0;
            while (lo < n->slotuse && key_less(n->get_key(lo), key)) ++lo;
            return lo;
        //}
    }
    ...
    {
        // if (0 && sizeof(n->get_key) > traits::binsearch_threshold)
        // {
        //     if (n->slotuse == 0) return 0;
        ...
        //     {
        //         int i = 0;
        //         while (i < n->slotuse && key_lessequal(n->get_key[i], key)) ++i;
        ...
        // {
            int lo = 0;
            while (lo < n->slotuse && key_lessequal(n->get_key(lo), key)) ++lo;
            return lo;
        //}
    }
...
public:
    ...
    bool exists(const key_type& key) const
    {
        ...
        return (slot < leaf->slotuse && key_equal(key, leaf->get_key(slot)));
    }
    ...
    size_type count(const key_type& key) const
    {
        ...
        while (leaf && slot < leaf->slotuse && key_equal(key, leaf->get_key(slot)))
        {
            ++num;
            if (++slot >= leaf->slotuse)
            {
                leaf->ensure_compressed();
                leaf = leaf->nextleaf;
                slot = 0;
            }
        }
...
public:
    ...
    inline compressed_btree(const compressed_btree& other)
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL),
          m_stats(other.m_stats),
          m_key_less(other.key_comp()),
          m_allocator(other.get_allocator())
...
private:
    ...
    struct node * copy_recursive(const node* n)
    {
        if (n->isleafnode())
        {
            ...
            std::copy(leaf->get_slotkey_array(), leaf->get_slotkey_array() + leaf->slotuse, newleaf->get_slotkey_array());
            data_copy(leaf->get_slotdata_array(), leaf->get_slotdata_array() + leaf->slotuse, newleaf->get_slotdata_array());
        ...
        else
        {
            ...
            std::copy(inner->get_slotkey_array(), inner->get_slotkey_array() + inner->slotuse, newinner->get_slotkey_array());
...
private:
    ...
    std::pair<iterator, bool> insert_start(const key_type& key, const data_type& value)
    {
        ...
        if (newchild)
        {
            ...
            newroot->get_key(0) = newkey;

            newroot->get_value(0) = m_root;
            newroot->get_value(1) = newchild;
    ...
    */
                                             ...
                                             key_type* splitkey, node** splitnode)
    {
            ...
            if (newchild)
            {
                ...
                if (inner->isfull())
                {
                    ...
                    if (slot == inner->slotuse + 1 && inner->slotuse < (*splitnode)->slotuse)
                    {
                        ...
                        inner->get_key(inner->slotuse) = *splitkey;
                ...
                std::copy_backward(inner->get_slotkey_array() + slot, inner->get_slotkey_array() + inner->slotuse,
                                   inner->get_slotkey_array() + inner->slotuse + 1);
                std::copy_backward(inner->get_slotdata_array() + slot, inner->get_slotdata_array() + inner->slotuse + 1,
                                   inner->get_slotdata_array() + inner->slotuse + 2);

                inner->get_key(slot) = newkey;
        ...
        else // n->isleafnode() == true
        {
            ...
            DeferCode c([&leaf, this](){
                leaf->ensure_compressed();
            });
            int slot = find_lower(leaf, key);

            if (!allow_duplicates && slot < leaf->slotuse && key_equal(key, leaf->get_key(slot))) {
            ...
            std::copy_backward(leaf->get_slotkey_array() + slot, leaf->get_slotkey_array() + leaf->slotuse,
                               leaf->get_slotkey_array() + leaf->slotuse + 1);
            data_copy_backward(leaf->get_slotdata_array() + slot, leaf->get_slotdata_array() + leaf->slotuse,
                               leaf->get_slotdata_array() + leaf->slotuse + 1);

            leaf->get_key(slot) = key;
            if (!used_as_set) leaf->get_value(slot) = value;
            leaf->slotuse++;
    ...
    void split_leaf_node(leaf_node* leaf, key_type* _newkey, node** _newleaf)
    {
        ...
        std::copy(leaf->get_slotkey_array() + mid, leaf->get_slotkey_array() + leaf->slotuse,
                  newleaf->get_slotkey_array());
        data_copy(leaf->get_slotdata_array() + mid, leaf->get_slotdata_array() + leaf->slotuse,
                  newleaf->get_slotdata_array());
        // fill the copied region with zeroes to make it more compressible
        memset(leaf->get_slotkey_array() + mid, 0, sizeof(key_type) * (leaf->slotuse - mid)); 
        memset(leaf->get_slotdata_array() + mid, 0, sizeof(data_type) * (leaf->slotuse - mid));
        leaf->slotuse = mid;
        leaf->nextleaf = newleaf;
        newleaf->prevleaf = leaf;

        *_newkey = leaf->get_key(leaf->slotuse - 1);
        *_newleaf = newleaf;
    }
    ...
    void split_inner_node(inner_node* inner, key_type* _newkey, node** _newinner, unsigned int addslot)
    {
        ...
        std::copy(inner->get_slotkey_array() + mid + 1, inner->get_slotkey_array() + inner->slotuse,
                  newinner->get_slotkey_array());
        std::copy(inner->get_slotdata_array() + mid + 1, inner->get_slotdata_array() + inner->slotuse + 1,
                  newinner->get_slotdata_array());

        inner->slotuse = mid;

        *_newkey = inner->get_key(mid);
        *_newinner = newinner;
    }
...
public:
    ...
    void bulk_load(Iterator ibegin, Iterator iend)
    {
        ...
        for (size_t i = 0; i < num_parents; ++i)
        {
            ...
            {
                n->get_key(s) = leaf->get_key(leaf->slotuse - 1);
                n->get_value(s) = leaf;
                leaf = leaf->nextleaf;
            }
            n->get_value(n->slotuse) = leaf;

            // track max key of any descendant.
            nextlevel[i].first = n;
            nextlevel[i].second = &leaf->get_key(leaf->slotuse - 1);
            leaf->ensure_compressed();
            leaf = leaf->nextleaf;
        ...
        for (int level = 2; num_parents != 1; ++level)
        {
            ...
            for (size_t i = 0; i < num_parents; ++i)
            {
                ...
                for (unsigned short s = 0; s < n->slotuse; ++s)
                {
                    n->get_key(s) = *nextlevel[inner_index].second;
                    n->get_value(s) = nextlevel[inner_index].first;
                    ++inner_index;
                }
                n->get_value(n->slotuse) = nextlevel[inner_index].first;
...
private:
                               ...
                               inner_node* parent, unsigned int parentslot)
    {
        if (curr->isleafnode())
        {
            ...
            DeferCode c([&, this](){
                leaf->ensure_compressed();
                leftleaf->ensure_compressed();
                rightleaf->ensure_compressed();
            });
            int slot = find_lower(leaf, key);

            if (slot >= leaf->slotuse || !key_equal(key, leaf->get_key(slot)))
            {
            ...
            std::copy(leaf->get_slotkey_array() + slot + 1, leaf->get_slotkey_array() + leaf->slotuse,
                      leaf->get_slotkey_array() + slot);
            data_copy(leaf->get_slotdata_array() + slot + 1, leaf->get_slotdata_array() + leaf->slotuse,
                      leaf->get_slotdata_array() + slot);
            memset(leaf->get_slotkey_array() + leaf->slotuse - 1, 0, sizeof(key_type)); // fill with zeroes to make it more compressible
            memset(leaf->get_slotdata_array() + leaf->slotuse - 1, 0, sizeof(data_type)); // fill with zeroes to make it more compressible
            leaf->slotuse--;
            ...
            if (slot == leaf->slotuse)
            {
                ...
                {
                    BTREE_ASSERT(parent->get_value(parentslot) == curr);
                    parent->get_key(parentslot) = leaf->get_key(leaf->slotuse - 1);
                }
                else
                {
                    if (leaf->slotuse >= 1)
                    {
                        BTREE_PRINT("Scheduling lastkeyupdate: key " << leaf->get_key(leaf->slotuse - 1));
                        myres |= result_t(btree_update_lastkey, leaf->get_key(leaf->slotuse - 1));
                    }
                    else
                    {
        ...
        else // !curr->isleafnode()
        {
            ...
            if (result.has(btree_update_lastkey))
            {
                if (parent && parentslot < parent->slotuse)
                {
                    ...
                    parent->get_key(parentslot) = result.lastkey;
                }
                else
                {
            ...
            if (result.has(btree_fixmerge))
            {
                ...
                std::copy(inner->get_slotkey_array() + slot, inner->get_slotkey_array() + inner->slotuse,
                          inner->get_slotkey_array() + slot - 1);
                std::copy(inner->get_slotdata_array() + slot + 1, inner->get_slotdata_array() + inner->slotuse + 1,
                          inner->get_slotdata_array() + slot);
                ...
                if (inner->level == 1)
                {
                    ...
                    slot--;
                    leaf_node* child = static_cast<leaf_node*>(inner->get_value(slot));
                    inner->get_key(slot) = child->get_key(child->slotuse - 1);
                }
            }
                                ...
                                inner_node* parent, unsigned int parentslot)
    {
        ...
        else // !curr->isleafnode()
        {
            ...
            while (slot <= inner->slotuse)
            {
                ...
                if (slot < inner->slotuse && key_less(inner->get_key(slot), iter.key()))
                    return result_t(btree_not_found);
            ...
            if (result.has(btree_fixmerge))
            {
                ...
                // this is the child slot invalidated by the merge
                BTREE_ASSERT(inner->get_value(slot)->slotuse == 0);

                free_node(inner->get_value(slot));
    ...
    result_t merge_leaves(leaf_node* left, leaf_node* right, inner_node* parent)
    {
        ...
        std::copy(right->get_slotkey_array(), right->get_slotkey_array() + right->slotuse,
                  left->get_slotkey_array() + left->slotuse);
        data_copy(right->get_slotdata_array(), right->get_slotdata_array() + right->slotuse,
                  left->get_slotdata_array() + left->slotuse);
    ...
    static result_t merge_inner(inner_node* left, inner_node* right, inner_node* parent, unsigned int parentslot)
    {
        ...
        left->get_key(left->slotuse) = parent->get_key(parentslot);
        left->slotuse++;

        // copy over keys and children from right
        std::copy(right->get_slotkey_array(), right->get_slotkey_array() + right->slotuse,
                  left->get_slotkey_array() + left->slotuse);
        std::copy(right->get_slotdata_array(), right->get_slotdata_array() + right->slotuse + 1,
                  left->get_slotdata_array() + left->slotuse);
    ...
    static result_t shift_left_leaf(leaf_node* left, leaf_node* right, inner_node* parent, unsigned int parentslot)
    {
        ...
        std::copy(right->get_slotkey_array(), right->get_slotkey_array() + shiftnum,
                  left->get_slotkey_array() + left->slotuse);
        data_copy(right->get_slotdata_array(), right->get_slotdata_array() + shiftnum,
                  left->get_slotdata_array() + left->slotuse);
        ...
        std::copy(right->get_slotkey_array() + shiftnum, right->get_slotkey_array() + right->slotuse,
                  right->get_slotkey_array());
        data_copy(right->get_slotdata_array() + shiftnum, right->get_slotdata_array() + right->slotuse,
                  right->get_slotdata_array());

        right->slotuse -= shiftnum;

        // fixup parent
        if (parentslot < parent->slotuse) {
            parent->get_key(parentslot) = left->get_key(left->slotuse - 1);
            return result_t(btree_ok);
        }
        else {  // the update is further up the tree
            return result_t(btree_update_lastkey, left->get_key(left->slotuse - 1));
        }
    }
    ...
    static void shift_left_inner(inner_node* left, inner_node* right, inner_node* parent, unsigned int parentslot)
    {
        ...
        std::copy(right->get_slotkey_array(), right->get_slotkey_array() + shiftnum - 1,
                  left->get_slotkey_array() + left->slotuse);
        ...
        // fixup parent
        parent->get_key(parentslot) = right->get_key(shiftnum - 1);
    ...
    static void shift_right_leaf(leaf_node* left, leaf_node* right, inner_node* parent, unsigned int parentslot)
    {
        ...
        std::copy_backward(right->get_slotkey_array(), right->get_slotkey_array() + right->slotuse,
                           right->get_slotkey_array() + right->slotuse + shiftnum);
        data_copy_backward(right->get_slotdata_array(), right->get_slotdata_array() + right->slotuse,
                           right->get_slotdata_array() + right->slotuse + shiftnum);

        right->slotuse += shiftnum;

        // copy the last items from the left node to the first slot in the right node.
        std::copy(left->get_slotkey_array() + left->slotuse - shiftnum, left->get_slotkey_array() + left->slotuse,
                  right->get_slotkey_array());
        data_copy(left->get_slotdata_array() + left->slotuse - shiftnum, left->get_slotdata_array() + left->slotuse,
                  right->get_slotdata_array());
    ...
    static void shift_right_inner(inner_node* left, inner_node* right, inner_node* parent, unsigned int parentslot)
    {
        ...
        right->get_key(shiftnum - 1) = parent->get_key(parentslot);

        // copy the remaining last items from the left node to the first slot in the right node.
        std::copy(left->get_slotkey_array() + left->slotuse - shiftnum + 1, left->get_slotkey_array() + left->slotuse,
                  right->get_slotkey_array());
        ...
        parent->get_key(parentslot) = left->get_key(left->slotuse - shiftnum);
...
private:
    ...
    static void print_node(std::ostream& os, const node* node, unsigned int depth = 0, bool recursive = false)
    {
        ...
        if (node->isleafnode())
        {
            ...
            for (unsigned int slot = 0; slot < leafnode->slotuse; ++slot)
            {
                os << leafnode->get_key(slot) << "  "; // << "(data: " << leafnode->get_value(slot) << ") ";
            }
            os << std::endl;
        }
        else
        {
            ...
            {
                os << "(" << innernode->childid[slot] << ") " << innernode->get_key(slot) << " ";
            }
...
private:
    ...
    void verify_node(const node* n, key_type* minkey, key_type* maxkey, tree_stats& vstats) const
    {
        ...
        if (n->isleafnode())
        {
            ...
            {
                assert(key_lessequal(leaf->get_key(slot), leaf->get_key(slot + 1)));
            }

            *minkey = leaf->get_key(0);
            *maxkey = leaf->get_key(leaf->slotuse - 1);
        ...
        else // !n->isleafnode()
        {
            ...
            {
                assert(key_lessequal(inner->get_key(slot), inner->get_key(slot + 1)));
            }
            ...
            for (unsigned short slot = 0; slot <= inner->slotuse; ++slot)
            {
                ...
                if (slot == 0)
                    *minkey = subminkey;
                    ...
                    assert(key_greaterequal(subminkey, inner->get_key(slot - 1)));

                if (slot == inner->slotuse)
                    *maxkey = submaxkey;
                else
                    assert(key_equal(inner->get_key(slot), submaxkey));
    ...
    void verify_leaflinks() const
    {
        ...
        while (n)
        {
            ...
            if (n->nextleaf)
            {
                assert(key_lessequal(n->get_key(n->slotuse - 1), n->nextleaf->get_key(0)));
