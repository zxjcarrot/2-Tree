...
class AntiCacheAdapter: public leanstore::StorageInterface<Key, Payload> {
public:
   ...
   AntiCacheAdapter(const string & block_storage_dir, double cache_size_gb, double storage_block_cache_size_gb, double maintain_lru_sampling_threshold = 1 ) : cache_capacity_bytes(cache_size_gb * 1024ULL * 1024ULL * 1024ULL), maintain_lru_sampling_threshold(maintain_lru_sampling_threshold){
   ...
   static constexpr std::size_t evict_block_size = 1024 * 4;
   ...
   void evict_bunch() {
      ...
      while (&lru_head != p) {
         if (block.size() >= evict_block_size) {
            break;
         }
   ...
   void bring_tuple_from_block_table(Key k, const TombstoneRecord & trec) {
      ...
      std::string block;
      block.reserve(evict_block_size);
      auto status = block_table->Get(options, rocksdb::Slice((const char *)key_bytes, key_len), &block);
      assert(status == rocksdb::Status::OK());
