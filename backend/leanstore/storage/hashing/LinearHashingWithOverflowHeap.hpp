...
class LinearHashTableWithOverflowHeap
{
  public:
   ...
   u64 get_splits() { return splits.load(); }
   u64 get_split_chain_length() { return split_length.load(); }
   u64 get_split_left_length() { return split_left_length.load(); }
...
private:
   ...
   std::atomic<u64> splits{0};
   std::atomic<u64> split_length{0};
   std::atomic<u64> split_left_length{0};
