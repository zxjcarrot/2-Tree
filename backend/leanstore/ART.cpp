...
ART::ART(std::size_t key_length) 
    : root(NULL), key_length(key_length), memory(0), num_items(0), 
      node4_count(0), node16_count(0), node48_count(0), node256_count(0), nullNode(NULL), loadKey(defaultLoadKey)
{}

ART::ART(std::size_t key_length, std::function<void(uintptr_t,uint8_t*)> loadKey) : root(NULL), key_length(key_length), memory(0), num_items(0), 
      node4_count(0), node16_count(0), node48_count(0), node256_count(0), nullNode(NULL), loadKey(loadKey) 
{}

ART::ART(std::size_t key_length, std::function<void(uintptr_t,uint8_t*)> loadKey, std::function<void(Node**, uintptr_t)> upsertFunc) 
    : root(NULL), key_length(key_length), memory(0), num_items(0), 
...
bool ART::lower_bound(uint8_t key[], unsigned keyLength, unsigned maxKeyLength, ARTIter* iter) {
	...
	loadKey(iter->val, (uint8_t*)iter->key_buf);
	iter->nextLeaf();
	return true;
    }
    else {
