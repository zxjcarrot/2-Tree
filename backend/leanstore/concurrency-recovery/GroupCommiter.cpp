...
void CRManager::groupCommiter()
{
   ...
   u64 ssd_offset_unaligned = ssd_offset;
   // -------------------------------------------------------------------------------------
   // Async IO
      ...
      for (u32 w_i = 0; w_i < workers_count; w_i++) {
         ...
         }
         {
            if (worker.group_commit_data.wt_cursor_to_flush > worker.wal_ww_cursor) {
               ...
               const u64 size_aligned = utils::upAlign4K(upper_offset - lower_offset);
               // -------------------------------------------------------------------------------------
               ensure(ssd_offset >= size_aligned);
