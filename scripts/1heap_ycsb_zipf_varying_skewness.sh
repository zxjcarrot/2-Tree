...
run_time=$RUNTIME
log_file="${RESULT_DIR_PREFIX}/1heap_ycsb_zipf_dram_skewness.log"
# empty log file
...
do
csv_file="${RESULT_DIR_PREFIX}/1heap_ycsb_zipf_dram_read_ratio_${read_ratio}_skewness_${zipf_factor}.csv"
$EXEC --trunc=1 --ycsb_tuple_count=$TUPLE_COUNT --dram_gib=5 --worker_threads=$WORKERS --index_type=IHeap  --top_component_dram_ratio=$top_component_dram_ratio --ycsb_read_ratio=$read_ratio --ycsb_update_ratio=$update_ratio --ssd_path=/flash1/xinjing/2-Tree/build/leanstore_data/leanstore --run_for_seconds=$run_time --ycsb_request_dist=zipfian2 --zipf_factor=$zipf_factor --lsmt_db_path=/flash1/xinjing/2-Tree/build/rocksdb --results_csv_file_path=$csv_file >> $log_file 2>&1
sleep $SLEEP_COUNT
done
done
