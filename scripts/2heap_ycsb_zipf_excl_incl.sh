...
do
run_time=$RUNTIME
log_file="2heap_ycsb_zipf_dram_${dram_budget}_heap_scan_exclusive.log"
# empty log file
...
do
$EXEC --trunc=1 --ycsb_tuple_count=$TUPLE_COUNT --dram_gib=$dram_budget --worker_threads=10 --index_type=2IHeap  --top_component_dram_ratio=$top_component_dram_ratio --ycsb_read_ratio=$read_ratio --ycsb_update_ratio=$update_ratio --ssd_path=/flash1/xinjing/2-Tree/build/leanstore_data/leanstore --run_for_seconds=$run_time --ycsb_request_dist=zipfian --zipf_factor=$zipf_factor --lsmt_db_path=/flash1/xinjing/2-Tree/build/rocksdb --cache_lazy_migration=30 --ycsb_2heap_eviction_index_scan=0 >> $log_file 2>&1
sleep $SLEEP_COUNT
done
done
done
...
do
run_time=$RUNTIME
log_file="2heap_ycsb_zipf_dram_${dram_budget}_indexed_heap_scan_exclusive.log"
# empty log file
cat /dev/null > $log_file
for read_ratio in 100 50
do
...
for zipf_factor in 0.9 # 0.7 0.8 0.9
do
$EXEC --trunc=1 --ycsb_tuple_count=$TUPLE_COUNT --dram_gib=$dram_budget --worker_threads=10 --index_type=2IHeap  --top_component_dram_ratio=$top_component_dram_ratio --ycsb_read_ratio=$read_ratio --ycsb_update_ratio=$update_ratio --ssd_path=/flash1/xinjing/2-Tree/build/leanstore_data/leanstore --run_for_seconds=$run_time --ycsb_request_dist=zipfian --zipf_factor=$zipf_factor --lsmt_db_path=/flash1/xinjing/2-Tree/build/rocksdb --xmerge --cache_lazy_migration=30 --ycsb_2heap_eviction_index_scan=1 >> $log_file 2>&1
sleep $SLEEP_COUNT
done
done
done
...
do
run_time=$RUNTIME
log_file="2heap_ycsb_zipf_dram_${dram_budget}_heap_scan_inclusive.log"
# empty log file
cat /dev/null > $log_file
for read_ratio in 100 50
do
...
do
run_time=$RUNTIME
log_file="2heap_ycsb_zipf_dram_${dram_budget}_indexed_heap_scan_inclusive.log"
# empty log file
cat /dev/null > $log_file
for read_ratio in 100 50
do
